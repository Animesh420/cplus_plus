...
void guess_numer_or_give_up(int number) {
    std::cout << "Guess the number.\n";
...
std::string check_which_digits_correct(int number, int guess) {
    ...
    for (size_t i = 0, stop = gs.length(); i < stop; ++i) {
        ...
        if (i < ns.length() && matches[i] != '*') {
            ...
            if (
                size_t idx = ns.find(guess_char, 0);
                idx != std::string::npos
            ) {
                matches[i] = '^';
                ns[idx] = '^';
...
void check_properties() {
    ...
    std::cout << "All checks successful\n";
}

int main() {
     check_properties();
    // guess_number(some_const_number());
    // guess_numer_or_give_up(some_const_number());
    auto make_message = [](int number, int guess) {
        return std::format("Your guess was too {}, numerical match {}\n", (guess < number ? "small" : "big"), check_which_digits_correct(number, guess));
    };
